...
void weno_minus_reference(const float *restrict _a, const float *restrict _b, const float *restrict _c,
						  const float *restrict _d, const float *restrict _e, float *const restrict out,
						  const int NENTRIES)
{
// #pragma omp parallel for
#pragma omp simd aligned(_a, _b, _c, _d, _e, out : 32)
	for (int i = 0; i < NENTRIES; ++i)
	{
		const float a = _a[i], b = _b[i], c = _c[i], d = _d[i], e = _e[i];
		...
		out[i] = omega0 * ((float)(1.0 / 3.) * a - (float)(7. / 6.) * b + (float)(11. / 6.) * c) +
				 omega1 * (-(float)(1. / 6.) * b + (float)(5. / 6.) * c + (float)(1. / 3.) * d) +
				 omega2 * ((float)(1. / 3.) * c + (float)(5. / 6.) * d - (float)(1. / 6.) * e);
	}
}
