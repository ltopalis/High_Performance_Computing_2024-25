...
inline void generate_random(float *arr, int NENTRIES)
{
	int ndiv4 = NENTRIES / 4;
	for (int i = 0; i < ndiv4; i++)
	{
		...
		_mm_store_ps(arr + 4 * i, a);
	}

	for (int i = ndiv4; i < NENTRIES; i++)
		arr[i] = rand() / (float)RAND_MAX;
}

float inline *myalloc(const int NENTRIES, const int verbose)
{
	...
	tmp = aligned_alloc(alignment_bytes, sizeof(float) * NENTRIES);
	// assert(result == 0);
